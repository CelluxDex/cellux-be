...
async function main() {
  try {
    logger.info('🚀 Starting Cellux DEX Backend (Pyth Oracle Integration)...');
    ...
    app.get('/', (req: Request, res: Response) => {
      res.json({
        success: true,
        message: 'Cellux DEX Backend - Pyth Oracle Price Service',
        version: '1.0.0',
        endpoints: {
          websocket: '/ws/price',
          prices: '/api/price',
    ...
    // Start server
    server.listen(PORT, () => {
      logger.success(`🎉 Cellux DEX Backend running on port ${PORT}`);
    ...
    logger.error('Failed to start Cellux DEX Backend:', error);
    process.exit(1);
  }
}
